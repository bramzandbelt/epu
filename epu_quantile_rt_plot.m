...
settings.plotType = 'line'; % 'line' or 'heat'
   ...
   case 'line'
      ...
      for i = 1
         for j = 1
            ...
            set(s, 'DefaultAxesFontName', 'Helvetica');
            set(s, 'DefaultTextFontName', 'Helvetica');
            
            xSize = 11.5; ySize = 11.5
            figWH = [xSize ySize];
            
            % Proportional positioning and scaling of figure on screen
            
            set(s,'Units','pixels')
            screenSize = get(0,'ScreenSize');
            screenWH = screenSize(3:4);
            pos = get(s,'Position');
            pos(3:4) = min(screenWH./figWH).*figWH;
            set(s,'Position',[pos(3)/2,pos(4)/2,pos(3),pos(4)])
            
            % Positioning of figure on paper
            set(s,'PaperUnits','centimeters')
            xLeft = (21-xSize)/2; yTop = (30-ySize)/2;
            set(s,'PaperPosition',[xLeft yTop xSize ySize])
...
%             pos = get(s, 'Position');
%             pos(3) = 18; % Select the width of the figure in [cm]
%             pos(4) = 18; % Select the height of the figure in [cm]
%             set(s, 'Position', pos);
...
%             set(hFig,'Position',screenSize);
            ...
            n = char(regexpdir(dataDir,['simDataMultiRt_nA1_to_nA1000.mat'],0));
            load(n)
...
%                   signRt = cell2mat(cellfun(@(x) x.sign,simDataMultiRt.rtQuantile(iSim),'UniformOutput',0));
...
%                   end
                  ...
                  hold on
                  for iV = 1:nQuantiles % Vincentile index
                     hP = plot(nAVec,y(:,iV),...
