...
if GRICE % Threshold is assumed to be variable across trials
   ...
   theta = nan(nSim,nA);
   parfor iSim = 1:nSim
      theta(iSim,:) = mTheta + sdTheta*(sqrt(2)*erfinv(2*(phiL+(phiR-phiL)*rand(1,nA))-1));
   end
   
else % Threshold is assumed to be constant across trials
   a = 0;
   b = Inf;
   ...
   % For simplicity, across-trial and across accumulator variability are assumed
   % to be identical
...
if GRICE % Rate is assumed to be constant across trials(but can vary between accumulators)

   muVec = kron(ones(nA,1),mV);
   sigmaVec = kron(ones(nA,1),sdV);
   
   
   covv = corr2cov(sigmaVec,rhoV);
   
   muVec = mV.*ones(nSim,nA)
   
   bla = mvnrnd(muVec,covv);
